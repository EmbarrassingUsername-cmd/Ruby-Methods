...
def caesar_cipher(string, change_integer)
  ...
  encrypted_character_array = split_string_array.map do |character|
    ...
    if char_code.between?(65, 90)
      character = if char_code + cipher_int < 91
                    (char_code + cipher_int).chr
                  else (((char_code + cipher_int) % 90) + 64).chr
                  end
    elsif char_code.between?(97, 122)
      character = if char_code + cipher_int < 123
                    (char_code + cipher_int).chr
                  else ((char_code + cipher_int) % 122 + 96).chr
                  end
    end
    character
  end
